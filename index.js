...
// Mở file data để coi data

// stock: Số lượng tồn kho
// discountPercentage: Số % giảm giá của sản phẩm
// rating: đánh giá




// Gợi ý (lúc nào cần thì search) :
// + js sort(), cách tìm min max trong 1 mảng js,
// + lọc các giá trị trung lặp trong array js

// data là 1 array chứa những sản phẩm của shop bán đồ của Kaka, hãy giúp Ngài đó làm những việc sau:

// C1: Shop có tổng cộng bao nhiêu sản phẩm
const product = data.map(function(item){
    return item.stock;
})
const stock = product.reduce(function(acc,vlaue){
    return acc + vlaue;
})

// C2: Nếu bán hết tất cả đồ trong kho (trừ đi tiền giảm giá),
//  thì Kaka kiếm được bao tiền
const buy = data.map(function(piceItem){
    return piceItem.price
})
const discount = data.map(function(price){
    return price.discountPercentage
})

var total = 0;

 for(var i = 0; i < buy.length && product.length ; i++ ){
     total =  ((buy[i]*discount[i]) * product[i])  + total ;
 }
// C3: Shop có tổng cộng bao nhiêu nhãn hàng(brand) ?
const list = data.map(function(nameBrand){
    return nameBrand.brand;
})
const listBrand = list.reduce(function(acc,vlaue){
    if(acc.indexOf(vlaue) === -1){
        acc.push(vlaue)
    }
    return acc
},[])
const allBrand = listBrand.length
// C4: Hãy tính tổng doanh thu của từng brand (trừ đi tiền giảm giá)
const totalBrand = data.map(function(item,index){
    return { brand:item.brand , 
            doanhThu : `${item.price * item.discountPercentage * item.stock}`
    }
})

// C5: + Sản phẩm nào có giá gốc lớn nhất ?
const maxPrice = buy.reduce(function(acc,vlaue){
    return (acc > vlaue) ? acc : vlaue
})
const productMax = data.find(function(item){
    return item.price === maxPrice ;
})
// console.log(productMax)
//     + Sản phẩm nào có phần trăm giảm giá nhỏ nhất ?
const minDiscount = discount.reduce(function(acc, vlaue){
    return (acc < vlaue) ? acc : vlaue
})

const minDiscountProduct = data.find(function(item){
    return item.discountPercentage === minDiscount ;
})
// console.log(minDiscountProduct)
// C6: Danh sách top 3 chiếc điện thoại mắc nhất ?
const priceMax = buy.sort(function compareNumbers(a, b) {
  return b - a;
});
    for(var i = 0 ; i < 3; i++){
var listProductMaxPrice = data.find(function(item){
    return item.price === priceMax[i];
})
    // console.log(listProductMaxPrice)
    }
